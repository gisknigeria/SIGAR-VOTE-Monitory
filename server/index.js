...
import { createId, createRateLimitState, normalizeText, sanitizeString, validateContentLength, validateCoordinates, validateEmail, validateExternalUrl, validateMediaPayload, validatePassword } from './security.js';
...
import { adminOnly, createAuth, isAdminRole, superAdminOnly } from './middleware/auth.js';
...
const store = createStore({
  ...
  jsonDb,
  saveJson,
  mappers: { toUser, toIncident, toNotification, toCamera, toMapLayer, toChatRoom, toChatMessage },
});
...
const { issueToken, sessionCookie, clearSessionCookie, authenticateToken, auth } = createAuth({
  secret,
  store,
  publicUser,
  asyncRoute,
...
app.post('/api/auth/login', asyncRoute(async (req, res) => {
  ...
  const email = ({ agent1: agent1Email, agent2: agent2Email }[loginValue] || loginValue);
...
io.on('connection', async socket => {
  ...
  try {
    await joinSocketToChatRooms(socket, store, socket.data.authUser, isAdminRole);
  } catch (error) {
    console.error('[chat] Could not join realtime rooms:', error.message);
    socket.emit('operation:error', { message: 'Realtime chat could not be initialized.' });
  }
