...
/**
 * Incident attachments are stored as private evidence references (id + metadata, no bytes),
 * so the actual media has to be fetched per-item from /evidence/:id. Older incidents created
 * before that pipeline still carry an inline `data` URL, which is used as-is.
 */
function EvidenceMedia({ item, index, token }) {
