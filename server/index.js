...
import {
  ...
  normalizeWardHistory,
  slugifyHistoricalArea,
} from "./historical-results.js";
...
jsonDb.users = jsonDb.users.map((user) => {
  ...
  if (user.role === "Admin")
    return {
      ...user,
      rank: "Admin",
      command: user.command || "Oyo State Command",
    };
  return user;
});
...
const toUser = (row) =>
  row && {
    ...
    lat: Number(row.lat) || 7.3775,
    lng: Number(row.lng) || 3.947,
  };
    ...
    const response = await fetch(
      `${baseUrl}/reverse?format=jsonv2&addressdetails=1&zoom=18&lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lng)}`,
      {
        headers: {
          ...
          "Accept-Language": "en",
          "User-Agent": `Oyo-Election-Monitor/1.0 (+${contactUrl})`,
        },
...
const isOyoStateNews = (value) => {
  ...
  const isHotelCompany =
    /\boyo[\s\-–—]*parent\b|\boyo[\s\-–—]*hotels?\b|\bhospitality\b|\bhotels?\b|\bprism\b|\bipo\b|\brooms?\b/i.test(
      text,
    );
  ...
  asyncRoute(async (req, res) => {
    ...
    if (year !== 2023 || !["presidential", "governor"].includes(office)) {
      return res
        ...
        .json({
          message:
            "Geographic history is currently available for Oyo 2023 Presidential and Governorship results.",
        });
    }
    try {
...
app.get(
  "/api/history/oyo/2023/presidential/lga/:lgaId",
  auth,
  rateLimit,
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      const payload = await fetchHistoricalResult(
        `/elections/2023/oyo/${lga.id}-${slugifyHistoricalArea(lga.name)}`,
      );
      return res.json({
...
app.get(
  "/api/history/oyo/2023/presidential/lga/:lgaId/ward/:wardCode",
  auth,
  rateLimit,
  asyncRoute(async (req, res) => {
    if (
    ...
    try {
      ...
      if (!lga)
        return res.status(404).json({ message: "LGA history was not found." });
      const payload = await fetchHistoricalResult(
        `/elections/2023/oyo/${lga.id}-${slugifyHistoricalArea(lga.name)}/${req.params.wardCode}`,
      );
      return res.json({
...
const configuredOyoIrevId = sanitizeString(
  process.env.IREV_OYO_ELECTION_ID || "",
).toLowerCase();
const IREV_OYO_ELECTION_ID = /^[a-f0-9]{24}$/.test(configuredOyoIrevId)
  ? configuredOyoIrevId
  : "";
...
const ensureIrevArchiveLoaded = () => {
  if (!irevArchiveLoadPromise)
    irevArchiveLoadPromise = Promise.all([
      ...
      if (
        IREV_OYO_ELECTION_ID &&
        archive?.electionId === IREV_OYO_ELECTION_ID &&
...
const loadOyoIrev = async (force = false) => {
  ...
  try {
    ...
    if (
      ...
      allUnits.length &&
      !isGovernorshipElectionName(remoteElectionName)
    ) {
      throw new Error(
        "The configured IReV election is not an Oyo governorship election",
      );
    }
    ...
    const mergedUploads = new Map(
      (irevOyoCache?.data?.uploads || []).map((upload) => [upload.id, upload]),
    );
    ...
    const data = {
      ...
      latestUploadAt:
        ...
        irevOyoCache?.data?.latestUploadAt ||
        "",
      uploads,
      fetchedAt: new Date().toISOString(),
    ...
    if (irevOyoCache?.data?.uploads?.length) {
      ...
      return {
        ...
        notice:
          "Live IReV is unavailable. Showing the last Oyo results saved on this server.",
      };
    }
    throw error;
  }
  ...
  asyncRoute(async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          message:
            "The official Oyo governorship IReV feed is temporarily unavailable.",
        });
    }
  }),
);
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const persistenceTask = irevOcrPersistQueue.then(() =>
      store.setSetting(IREV_OYO_OCR_KEY, Object.fromEntries(irevOcrCache)),
    );
  ...
  asyncRoute(async (req, res) => {
        ...
        const articles = (payload.articles || [])
          ...
          .filter(
            (item) =>
              item.title &&
              item.url &&
              isOyoStateNews(`${item.title} ${item.description}`),
          )
          .sort(
            (a, b) =>
    ...
    const queries = [
      ...
      ...configuredParties.map(
        (party) => `"Oyo State" political party "${party}"`,
      ),
    ];
    const feeds = queries
      .map(
    ...
    if (process.env.GEMINI_API_KEY) {
      ...
      try {
        ...
        return res.json({
          ...
          summary: `Summary service unavailable. ${articles.length} Oyo-related headlines were retrieved. Review the linked sources, prioritize the newest reports, and verify claims against official Oyo State and INEC channels before acting.`,
        });
      }
    }
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const email =
      { agent1: agent1Email, agent2: agent2Email }[loginValue] || loginValue;
    const password = String(req.body.password || "");
    if (!validateEmail(email) || !password || password.length > 1024)
      return res
        .status(400)
...
app.put(
  ...
  asyncRoute(async (req, res) => {
    ...
    const changes = {
      ...
      lng: Number(req.body.lng ?? target.lng) || 3.947,
    };
    const existing = (await store.users()).find(
      (user) =>
        user.id !== target.id &&
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const rawEntries = (Array.isArray(req.body.results) ? req.body.results : [])
      .map((item) => ({
      ...
      ...rawEntries
        .reduce(
          (map, item) =>
            map.set(item.party, {
              party: item.party,
              votes: (map.get(item.party)?.votes || 0) + item.votes,
            }),
          new Map(),
        )
        .values(),
    ...
    if (!entries.length)
      return res
        ...
        .json({
          message:
            "Select at least one uploaded party and enter its vote count",
        });
    ...
    const state = normalizeRegistrationState(
      fieldRole ? req.user.state : req.body.state || req.user.state,
    );
  ...
  asyncRoute(async (req, res) => {
    ...
    const incident = {
      ...
      severity: ["Low", "Medium", "High", "Critical"].includes(
        ...
        ? req.body.severity
        : "High",
      status: ["Open", "In Progress", "Resolved", "Submitted"].includes(
        req.body.status,
      )
        ? req.body.status
        : "Open",
...
app.put(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (
      ...
      !["Open", "In Progress", "Resolved", "Submitted"].includes(patch.status)
    )
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const visibleTo = [
      ...new Set([...(incident.visibleTo || []), assignedUserId]),
    ];
    ...
    const notification = await store.createNotification({
      ...
      message,
      incidentType: incident.reportType || "Incident assignment",
      read: false,
      createdAt: new Date().toISOString(),
    });
...
app.get(
  ...
  auth,
  rateLimit,
  asyncRoute(async (_, res) => res.json(await store.cameras())),
);
app.post(
  "/api/cameras",
  auth,
  ...
  asyncRoute(async (req, res) => {
    ...
    const lat = Number(req.body.lat ?? 7.3775);
    const lng = Number(req.body.lng ?? 3.947);
    if (!validateCoordinates(lat, lng))
...
app.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    io.emit("camera:deleted", req.params.id);
    res.status(204).end();
  }),
);
app.get(
...
app.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    io.emit("map-layer:deleted", req.params.id);
    res.status(204).end();
  }),
);
app.use(
...
io.on("connection", async (socket) => {
  ...
  socket.on("camera:share:start", (payload) => {
    ...
    for (const client of io.sockets.sockets.values())
      if (isAdminRole(client.data.authUser))
        client.emit("camera:share:start", safePayload);
    if (validateCoordinates(lat, lng)) {
      reverseLocation(lat, lng)
        .then((location) => {
          ...
          for (const client of io.sockets.sockets.values())
            if (isAdminRole(client.data.authUser))
              client.emit("camera:share:start", updated);
        })
        .catch((error) =>
          console.error(
  ...
  socket.on("camera:share:stop", () => {
    const userId = socket.data.authUser.id;
    activeCameraShares.delete(userId);
    socket.broadcast.emit("camera:share:stop", { userId });
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (
      ...
      socket.broadcast.emit("camera:share:stop", { userId: user.userId });
    }
  });
});
...
server.listen(process.env.PORT || 5000, "0.0.0.0", () =>
  console.log(
    `Election Monitoring Command API listening on port ${process.env.PORT || 5000}`,
  ),
);
