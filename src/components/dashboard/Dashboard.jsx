      ...
      <main className="results-center-body">
        ...
        {view === "irev" && <section className="irev-pilot-card">
          ...
          {irevPilot && <>
            ...
            {irevPreview && <div className="irev-preview-backdrop" onClick={() => setIrevPreview(null)}><section className="irev-preview-modal" onClick={(event) => event.stopPropagation()}><header><div><span className="eyebrow">INEC IREV RESULT SHEET</span><h2>{irevPreview.puCode}</h2><p>{irevPreview.lga} · {irevPreview.ward} · {irevPreview.pollingUnit}</p></div><button type="button" className="icon-btn" onClick={() => setIrevPreview(null)} aria-label="Close result preview"><FaTimes /></button></header><div className="irev-preview-body">{irevPreview.imageUrl ? <div className="irev-preview-image"><img src={irevPreview.imageUrl} alt={`INEC IReV result sheet for ${irevPreview.puCode}`} /></div> : <div className="irev-preview-image irev-result-sheet-fallback"><span className="eyebrow">PREPARED ARCHIVE</span><h3>{irevPreview.pollingUnit}</h3>{(() => { const result = irevResultRows.find(row => row.puCode === irevPreview.puCode); return result ? <div className="irev-fallback-votes">{result.results.map(item => <div key={item.party}><strong>{item.party}</strong><b>{Number(item.votes || 0).toLocaleString()}</b></div>)}</div> : <p className="muted">No vote figures are available for this polling unit.</p>; })()}</div>}<aside><div className="irev-preview-actions"><a href={irevPreview.imageUrl || irevPreview.sourceUrl} target="_blank" rel="noreferrer">{irevPreview.imageUrl ? "Open image" : "Open official source"}</a></div><h3>{irevPreview.imageUrl ? "Original result sheet" : "Saved result record"}</h3><p className="muted">{irevPreview.imageUrl ? "This image is shown exactly as published on IReV." : "The saved demo archive contains the polling-unit result figures but no image file. The official source link remains available above."}</p></aside></div></section></div>}
          </>}
