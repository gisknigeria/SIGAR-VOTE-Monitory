...
function ResultsCenter({ incidents, onClose, authToken }) {
  ...
  return (
    <div className="results-center">
      ...
      <main className="results-center-body">
        ...
        {view === "news" && <section className="result-table-card"><div className="result-table-title"><div><h2>Election News Monitor</h2><p>Aggregated headlines covering elections, INEC, political parties, voting, and security.</p></div><div><button className="primary" onClick={() => { setNews([]); setNewsSummary(""); setNewsSummaryError(""); setView("news"); }}>Refresh</button> <button className="secondary" disabled={!news.length || newsSummaryLoading} onClick={() => { setNewsSummaryLoading(true); setNewsSummaryError(""); request("/news/summary", authToken, { method: "POST", body: JSON.stringify({ articles: news }) }).then((x) => { setNewsSummary(x.summary || "No summary available yet."); if (x.provider === "local") setNewsSummaryError("AI provider not configured, so a local fallback summary was generated."); else setNewsSummaryError(""); }).catch((error) => { setNewsSummary(""); setNewsSummaryError(error.message || "The AI summary request failed."); }).finally(() => setNewsSummaryLoading(false)); }}> {newsSummaryLoading ? "Working…" : "AI Summary"}</button></div></div>{newsSummary && <p className="news-summary">{newsSummary}</p>}{newsSummaryError && <p className="muted">{newsSummaryError}</p>}{newsLoading ? <p>Loading current headlines…</p> : <div className="news-list">{news.map(item => <article className="news-item" key={item.url}><a href={item.url} target="_blank" rel="noreferrer"><h3>{item.title}</h3></a><small>{item.source} · {item.publishedAt || "Recent"}</small></article>)}{!news.length && <p>No current headlines available.</p>}</div>}</section>}
        {view !== "breakdown" && view !== "news" && <section className="result-table-card" style={{ marginBottom: 16 }}><div className="result-table-title"><div><h2>Party performance & operational outlook</h2><p>Neutral analysis based only on submitted results and operational reports.</p></div><div><select value={focusParty} onChange={e => setFocusParty(e.target.value)}><option value="">Select a party</option>{summary.partyNames.map(p => <option key={p} value={p}>{p}</option>)}</select><button className="primary" disabled={aiOutlookLoading} onClick={() => { const safeIncidents = incidents.slice(0, 100).map(item => ({ id: item.id, title: item.title, description: String(item.description || "").slice(0, 500), reportType: item.reportType, severity: item.severity, status: item.status, lga: item.lga, ward: item.ward, pollingUnit: item.pollingUnit, createdAt: item.createdAt })); setAiOutlookLoading(true); request("/analysis/ai", authToken, { method: "POST", body: JSON.stringify({ context: { projection: forecast, selectedParty: focusParty, partyAnalysis, incidents: safeIncidents, actions } }) }).then(x => setAiOutlook(x.analysis || "No AI analysis returned.")).catch(e => setAiOutlook(e.message || "AI analysis unavailable.")).finally(() => setAiOutlookLoading(false)); }}>{aiOutlookLoading ? "Analyzing…" : "AI Operational Analysis"}</button></div></div>{partyAnalysis && <p>{focusParty}: <b>{partyAnalysis.votes.toLocaleString()}</b> votes, leading in <b>{partyAnalysis.wards}</b> wards and <b>{partyAnalysis.lgas}</b> LGAs. Related incident mentions: <b>{partyAnalysis.incidents}</b>.</p>}{aiOutlook && <p className="news-summary">{aiOutlook}</p>}<ul>{actions.map(action => <li key={action}>{action}</li>)}</ul></section>}
...
function Dashboard({ session, onLogout, onSessionUpdate }) {
  ...
  return (
    <main className={`app-shell role-${session.user.role.toLowerCase().replaceAll(" ", "-")}`}>
      ...
      {resultsOpen && <ResultsCenter incidents={incidents} onClose={() => setResultsOpen(false)} authToken={session.token} />}
