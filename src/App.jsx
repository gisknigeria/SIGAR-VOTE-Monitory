  ...
  return (
    <div className="results-center">
      ...
      <main className="results-center-body">
        ...
        {view === "news" && <section className="result-table-card"><div className="result-table-title"><div><h2>Oyo Politics & Election News</h2><p>Oyo State politics, INEC, elections, parties, campaigns, polling, and governance.</p></div><div><button className="primary" onClick={() => { setNews([]); setNewsSummary(""); setNewsSummaryError(""); setView("news"); }}>Refresh</button> <button className="secondary" disabled={!news.length || newsSummaryLoading} onClick={() => { setNewsSummaryLoading(true); setNewsSummaryError(""); request("/news/summary", authToken, { method: "POST", body: JSON.stringify({ articles: news }) }).then((x) => { setNewsSummary(x.summary || "No summary available yet."); if (x.provider === "local") setNewsSummaryError("AI was unavailable, so a local fallback summary was generated."); else setNewsSummaryError(""); }).catch((error) => { setNewsSummary(""); setNewsSummaryError(error.message || "The AI summary request failed."); }).finally(() => setNewsSummaryLoading(false)); }}> {newsSummaryLoading ? "Working…" : "AI Summary"}</button></div></div>{newsSummary && <div className="news-summary">{cleanAiText(newsSummary)}</div>}{newsSummaryError && <p className="muted">{newsSummaryError}</p>}{newsLoading ? <p>Loading current headlines…</p> : <div className="news-list">{news.map(item => <article className="news-item" key={item.url}><a href={item.url} target="_blank" rel="noreferrer"><h3>{item.title}</h3></a><small>{item.source} · {item.publishedAt ? new Date(item.publishedAt).toLocaleString() : "Recent"}</small></article>)}{!news.length && <p>No current Oyo political headlines available.</p>}</div>}</section>}
